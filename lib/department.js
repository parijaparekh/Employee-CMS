...
class Department {
    ...
    async budgetForDepartment(departmentlist){
        this.department_questions[1].choices = departmentlist;
        const answers = await this.inquirer.prompt(this.department_questions[1]);
        const sql = 'select sum(r.salary) as utilized_budget, d.dept_name as department_name from roles r, department d, employee e where e.role_id = r .id and r.department_id = d.id and d.id = ? Group By d.id';
        const params = answers.department_id;
